...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Joomla Developer Tool", epilog="Created by Rogachev Sergey", usage="%(prog)s [--init] | [--print] | [--create [options]]")
    ...
    if args_vars['init']:
        ...
        if(not author):
            print(
                "Configuration file does not exist. Run tool with '-i' option")
            return 0

        if args_vars['struct']:
            jcomponent.create_component(
                args_vars['name'], author, args_vars['description'], tool_conf)
            return 1

        if args_vars['model'] or args_vars['view'] or args_vars['controller']:
            dst = filter(
                lambda x: x is not None, [args_vars['backend'], args_vars['frontend']])
            if sys.version_info.major < 3:
                try:
                    dst = dst[0]
                except Exception:
                    print(
                        "Select destination: -b for back-end, -f for front-end")
                    return 0
            else:
                try:
                    dst = next(dst)
        ...
        if args_vars['model'] or args_vars['controller']:
            jclass = filter(
                lambda x: x is not None, [args_vars['L'], args_vars['A'], args_vars['F']])
            if sys.version_info.major < 3:
                try:
                    jclass = jclass[0]
                except Exception:
                    print(
                        "Select class model: -L for JModelList/JControllerLegacy, -F for JModelForm/JControllerForm or -A for JModelAdmin/JControllerAdmin")
                    return 0
            else:
                try:
                    jclass = next(jclass)
        ...
        if args_vars['model']:
            jcomponent.create_model(
                args_vars['name'], author, jclass, dst, tool_conf)

        elif args_vars['view']:
            jcomponent.create_view(args_vars['name'], author, dst, tool_conf)

        elif args_vars['controller']:
            jcomponent.create_controller(
                args_vars['name'], author, jclass, dst, tool_conf)
